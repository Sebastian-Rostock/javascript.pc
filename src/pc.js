...
function pc() { // (): pcNS<pc>
	...
	var override = function() {// (...: String; Method: Function): xpcCore<xpcThis>
		...
		if (method) while ((i < size) && superNode)
			superNode = (_this = superNode)[key = list[i++]];
	...
	pc.Is = function(Type) { // (Type: pcType<?>; ...: pcType<?>): Boolean
		for (var i = 1, map = Type.constructor.constructor; i < arguments.length; i++)
			if (!map[arguments[i].constructor.constructor.constructor]) return false;
		return true;
	};
	pc.Of = function(Spec, Simple) {// <GThis: pcThis<GType>; GType: pcType<GThis; GSpec>; GSpec: pcSpec<GThis; GType; ?; ?>>(Spec: GSpec; Simple: Boolean): GType
		if (Simple) {
			var baseS = {}, thisS = {}, typeT = function() { // (): Void
				...
				This: thisS,
				Type: typeT,
				Super: function() { // (...: spcType<?>): spcCore<spcThis>
					for (var i = 0, key, item, list = arguments; i < list.length; i++)
						if (!baseS[key = (item = list[i].constructor).constructor.constructor]) (baseS[key] = item).call(coreS, thisS, typeT);
					return this;
				},
				Override: override
			}, typeS = (baseS[((((typeT.prototype = thisS).constructor = typeT).constructor = Spec).constructor = baseS).constructor = String(++id)] = Spec).call(coreS, thisS, typeT) || function() {
			};
			...
			coreThis.Super = function() { // (...: xpcType<?>): xpcCore<xpcThis>
				for (var i = 0, key, item, list = arguments, node; i < list.length; i++) {
					...
					if (!base[key = (item = list[i]).constructor.constructor.constructor]) superNode[0] = (base[key] = item).constructor.call(this, this.This, this.Type);
					superNode = node;
				}
				return this;
			};
	...
	pc.MapToString = function(Map) { // (Map: String{}): String
		if (!Map) return 'null';
		var list = [];
		for (var key in Map)
			list.push(pc.TextToString(key) + ': ' + pc.TextToString(String(Map[key])));
		return (list.length ? '{\n  ' + list.join(',\n  ') + '\n}' : '{}');
	};
